...
BEGIN
    ...
    IF v_len <= 2 THEN
        v_masked_email := rpad(substring(v_local_part from 1 for 1), v_len, '*') || '@' || v_domain;
    ELSE
        v_masked_email := substring(v_local_part from 1 for 2) || repeat('*', v_len - 2) || '@' || v_domain;
    END IF;
...
-- ==========================================
-- 6. 회원탈퇴 시 auth.users 계정 영구 삭제 함수(RPC)
-- ==========================================
...
BEGIN
    ...
    IF v_claims IS NULL OR v_claims = '' THEN
        RAISE EXCEPTION '인증 세션이 유효하지 않습니다.';
    END IF;
    ...
    -- 부모 auth.users 행을 직접 파기 (ON DELETE CASCADE 정책에 따라 public.profiles 도 함께 지워짐)
